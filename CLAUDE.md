...
- **Parakeet chunk pool** — long audio is split into chunks (180s with native VAD, 45s without) and, when `config.transcription.chunk_workers >= 2` or the auto default, dispatched across a pool of parakeet sidecars (one subprocess + one Unix socket per worker). Each worker loads its own model replica (~1.2 GB), so the pool trades RAM for wall-clock speedup. Per-chunk completion emits `TranscribeChunkCompleted` events to `~/.minutes/events.jsonl` for live progress tailing. See `crates/core/src/parakeet_pool.rs` and `transcribe::transcribe_chunk_ranges_parakeet_pool`.
- **Overlap + pairwise merge** (parakeet only) — when `config.transcription.chunked.overlap_seconds > 0` (default 10s), consecutive chunks share an overlap region. After the pool finishes, a three-tier merge runs per seam: tier 1 exact token match → concat; tier 2 fuzzy LCS within `fuzzy_threshold` (default 0.15); tier 3 agent-assisted via `claude -p --output-format json --json-schema` with cached results under `~/.minutes/merge-cache/`. Deterministic fallback ("trust chunk N+1's head text") kicks in when the agent is missing/times out/returns bad JSON, or when `strategy = "deterministic"`. Each seam emits a `TranscribeSeamResolved { tier, agent_invocations }` event. See `crates/core/src/merge.rs`.
